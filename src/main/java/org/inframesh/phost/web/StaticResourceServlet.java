...
public class StaticResourceServlet extends HttpServlet {
	...
	public static class StaticResourceFile implements LookupResult {
		...
		public StaticResourceFile(long lastModified, String mimeType, int contentLength, boolean acceptsDeflate, URL url) {
	...
	protected LookupResult lookupNoCache(HttpServletRequest req) {
		...
		if(isForbidden(path)) {
			log.error("Forbidden to access resource(may be not static resource): " + path);
			return new Error(HttpServletResponse.SC_FORBIDDEN, "Forbidden");
		}

		log.debug("Handle static resource: " + path);
		...
		if(realpath != null) {
			...
			if(!f.isFile())
				...
				return new StaticResourceFile(f.lastModified(),mimeType,(int)f.length(),acceptsDeflate(req),url);
		} else {
			try {
				// Try as a JAR Entry
				...
				if (ze != null) {
					if(ze.isDirectory())
						...
						return new StaticResourceFile(ze.getTime(),mimeType,(int)ze.getSize(),acceptsDeflate(req),url);
				} else
					// Unexpected?
					return new StaticResourceFile(-1,mimeType,-1,acceptsDeflate(req),url);
	...
	protected boolean isForbidden(String path) {
		...
		String regex = excludePath.replace("\\s", "").replace(".", "\\.").replace("|", "\\|").replace("*", ".*").replace(",", "|");
		return lpath.startsWith("/web-inf/") || lpath.startsWith("/meta-inf/") || lpath.matches(regex);
	}
	
	public void init() throws ServletException{
		String excludePath = this.getInitParameter("excludePath");
		if(excludePath != null) {
